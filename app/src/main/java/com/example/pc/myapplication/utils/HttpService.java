...
public class HttpService {
     ...
     * 父母端的每日签到
     */
    private static OnEveryDaySignInRequestResponseListener mEveryDaySignInListener;

    public interface OnEveryDaySignInRequestResponseListener {
        void OnEveryDaySignInSuccessResponse(JSONArray jsonArray);

        void OnEveryDaySignInFailedResponse(String errorResult);
    }

    public static void DoEveryDaySignInRequest(OnEveryDaySignInRequestResponseListener listener) {
        mEveryDaySignInListener = listener;
        Response.Listener<JSONArray> responseListener = new Response.Listener<JSONArray>() {
            @Override
            public void onResponse(JSONArray jsonArray) {
                mEveryDaySignInListener.OnEveryDaySignInSuccessResponse(jsonArray);
            }
        };
        Response.ErrorListener errorListener = new Response.ErrorListener() {
            @Override
            public void onErrorResponse(VolleyError volleyError) {
                mEveryDaySignInListener.OnEveryDaySignInFailedResponse(volleyError.getMessage());
            }
        };
        HttpApi.DoJsonArrayRequest(Request.Method.GET, AppConstant.EVERYDAY_SIGN_IN_URL, null, responseListener, errorListener);
    }

    /**
     * 获取种子信息
     */
    private static OnGetSeedInfoRequestListener mGetSeedInfoListener;

    public interface OnGetSeedInfoRequestListener {
        void OnGetSeedInfoSuccessResponse(JSONArray jsonArray);

        void OnGetSeedInfoFailedResponse(String errorResult);
    }

    public static void DoGetSeedInfoRequest(OnGetSeedInfoRequestListener listener) {
        mGetSeedInfoListener = listener;
        Response.Listener<JSONArray> responseListener = new Response.Listener<JSONArray>() {
            @Override
            public void onResponse(JSONArray jsonArray) {
                mGetSeedInfoListener.OnGetSeedInfoSuccessResponse(jsonArray);
            }
        };
        Response.ErrorListener errorListener = new Response.ErrorListener() {
            @Override
